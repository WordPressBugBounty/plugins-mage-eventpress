...
if (! class_exists('MPWEM_Event_Edit_Page')) {
	...
	class MPWEM_Event_Edit_Page
	{
		...
		private const NONCE_ACTION_SWITCH = 'mpwem_switch_event_editor';
		private const CLASSIC_BYPASS_QUERY = 'mpwem_classic';
		private const EVENT_MODE_META_KEY = '_mpwem_event_edit_mode';
...
?>
		...
		{
			if ($post_id > 0) {
				$mode = get_post_meta($post_id, self::EVENT_MODE_META_KEY, true);
		...
		private function set_event_edit_mode(int $post_id, string $mode): void
		{
			...
			update_post_meta($post_id, self::EVENT_MODE_META_KEY, $mode);
		}
		...
		private function get_modern_edit_url(int $post_id = 0): string
		{
			if ($post_id > 0) {
		...
		private function get_editor_switch_url(string $mode, int $post_id = 0): string
		{
			$args = [
				'action' => 'mpwem_switch_event_editor',
				'mode'   => $mode,
			];
			...
			return wp_nonce_url(
				...
				self::NONCE_ACTION_SWITCH
			);
		}

		private function is_classic_event_post_screen(): bool
		{
		...
		private function render_setting_roles_field(array $selected_roles, bool $is_active): void
		{
			...
			<div class="mpwem-event-setting-card__item mpwem-event-setting-card__item--roles <?php echo esc_attr($is_active ? 'mActive' : ''); ?>" data-collapse="#mep_member_only_event">
				...
				<div class="mpwem-taxonomy-checklist mpwem-taxonomy-checklist--roles">
					<label class="mpwem-taxonomy-checklist__item">
						<input type="checkbox" data-no-mpwem-switch="1" name="mep_member_only_user_role[]" value="all" <?php checked(in_array('all', $selected_roles, true)); ?> />
					...
					<?php foreach ($editable_roles as $role => $details) : ?>
						<label class="mpwem-taxonomy-checklist__item">
							<input type="checkbox" data-no-mpwem-switch="1" name="mep_member_only_user_role[]" value="<?php echo esc_attr($role); ?>" <?php checked(in_array($role, $selected_roles, true)); ?> />
		...
		private function render_slug_card(int $post_id): void
		{
			...
			<div class="mpwem-card mpwem-taxonomy-card mpwem-taxonomy-card--slug">
				...
				<div class="mpwem-card__body">
					<div class="mpwem-field">
						<label class="mpwem-field" for="post_name">
							<span class="mpwem-field__label"><?php esc_html_e('Slug', 'mage-eventpress'); ?></span>
							<input
								id="post_name"
								class="mpwem-input"
								type="text"
								...
								placeholder="<?php esc_attr_e('event-slug', 'mage-eventpress'); ?>" />
						</label>
					</div>
					<div class="mpwem-taxonomy-card__meta">
						<p class="mpwem-taxonomy-card__help">
							<?php esc_html_e('Permalink:', 'mage-eventpress'); ?> 
							<a id="mpwem_slug_preview" href="<?php echo esc_url(get_permalink($post_id)); ?>" target="_blank" rel="noopener noreferrer" data-original-url="<?php echo esc_url(get_permalink($post_id)); ?>"><?php echo esc_url(get_permalink($post_id)); ?></a>
						</p>
					</div>
				</div>
			</div>
		...
		private function render_event_setting_options(int $post_id): void
		{
			...
			<div class="mpwem-event-setting-card__grid">
				...
				$this->render_setting_toggle_field(
					...
					$is_member_only,
					__('Restrict event access to logged-in members with specific roles.', 'mage-eventpress'),
					'member_only'
				);
		...
		private function render_category_card(int $post_id): void
		{
			$terms = $this->get_taxonomy_terms('mep_cat');
			$selected_ids = $this->get_selected_term_ids($post_id, 'mep_cat');
			?>
		...
		private function render_organizer_card(int $post_id): void
		{
			$terms = $this->get_taxonomy_terms('mep_org');
			$selected_ids = $this->get_selected_term_ids($post_id, 'mep_org');
			?>
		...
		private function render_tag_card(int $post_id): void
		{
				...
				<div class="mpwem-card__body">
					<div class="mpwem-taxonomy-card__field mpwem-taxonomy-card__field--tags">
						<label class="mpwem-field" for="mpwem_event_tags">
							<span class="mpwem-field__label"><?php esc_html_e('Tags', 'mage-eventpress'); ?></span>
							<input
								id="mpwem_event_tags"
								name="mep_tag_names"
								type="text"
		...
		private function render_event_settings_card(int $post_id): void
		{
			?>
			<div class="mpwem-card mpwem-taxonomy-card mpwem-taxonomy-card--settings">
				<div class="mpwem-card__head">
					<h2><?php esc_html_e('Event Settings', 'mage-eventpress'); ?></h2>
					<p><?php esc_html_e('Control core event identity and access settings.', 'mage-eventpress'); ?></p>
				</div>
				<div class="mpwem-card__body">
		...
		public function filter_row_actions($actions, $post)
		{
			...
			$default_is_modern = $this->is_modern_mode_enabled((int) $post->ID);
			...
			if ($default_is_modern) {
				$actions['classic'] = sprintf(
					'<a href="%s">%s</a>',
					esc_url($this->get_editor_switch_url('classic', (int) $post->ID)),
					esc_html__('Classic Editor', 'mage-eventpress')
				);
			} else {
				$actions['modern'] = sprintf(
					'<a href="%s">%s</a>',
					esc_url($this->get_editor_switch_url('modern', (int) $post->ID)),
		...
		public function render_classic_switch_title_button(): void
		{
			...
			$button_text      = __('Edit with Modern Editor', 'mage-eventpress');
			$nonce_field_html = wp_nonce_field(self::NONCE_ACTION_SWITCH, '_mpwem_switch_nonce', true, false);
		?>
			<style>
				...
				<script>
					jQuery(function ($) {
						var $titleActions = $('.wrap .page-title-action').first();
						if (! $titleActions.length || $('.mpwem-title-switch-button').length) {
							return;
						}

						var formHtml = ''
							+ '<form class="mpwem-title-switch-form" method="post" action="' + <?php echo wp_json_encode($form_action); ?> + '">'
							+ '<input type="hidden" name="action" value="mpwem_switch_event_editor">'
							+ '<input type="hidden" name="mode" value="modern">'
							+ '<input type="hidden" name="post_id" value="' + <?php echo wp_json_encode((string) $post_id); ?> + '">'
							+ <?php echo wp_json_encode($nonce_field_html); ?>
							+ '</form>';

					var $form = $(formHtml);
					var $submit = $('<button>', {
						type: 'submit',
						'class': 'mpwem-title-switch-button'
					});
					...
					$submit.append($('<span>', {
						'class': 'mpwem-title-switch-label',
						text: <?php echo wp_json_encode($button_text); ?>
					}));

					$form.append($submit);
					$titleActions.after($form);
				});
			</script>
		<?php
		}
		...
		{
			if (! $this->can_manage_events()) {
				wp_die(esc_html__('Sorry, you are not allowed to switch the event editor.', 'mage-eventpress'));
			}

			check_admin_referer(self::NONCE_ACTION_SWITCH, isset($_REQUEST['_mpwem_switch_nonce']) ? '_mpwem_switch_nonce' : '_wpnonce');

			$mode    = isset($_REQUEST['mode']) ? sanitize_key(wp_unslash($_REQUEST['mode'])) : '';
			$post_id = isset($_REQUEST['post_id']) ? absint($_REQUEST['post_id']) : 0;

			if (! in_array($mode, ['modern', 'classic'], true)) {
				wp_die(esc_html__('Invalid editor mode.', 'mage-eventpress'));
			}
			...
			if ($post_id > 0) {
				$this->set_event_edit_mode($post_id, $mode);
			}

			$redirect_url = $mode === 'classic'
				? ($post_id > 0 ? $this->get_classic_edit_url($post_id) : admin_url('post-new.php?post_type=' . self::POST_TYPE . '&' . self::CLASSIC_BYPASS_QUERY . '=1'))
				: $this->get_modern_edit_url($post_id);
		...
		public function render_page(): void
		{
			...
			$classical_url = $post_id ? $this->get_editor_switch_url('classic', $post_id) : '';
								...
								<?php if (($is_published && ($can_publish || $can_trash)) || (! $is_published && $can_trash && $can_publish)) : ?>
									<div class="mpwem-status-actions__menu-wrap">
										...
										<div class="mpwem-status-actions__menu" role="menu">
											<?php if ($frontend_url) : ?>
												<a href="<?php echo esc_url($frontend_url); ?>" class="mpwem-status-actions__menu-item" target="_blank">
													<span class="dashicons dashicons-external" aria-hidden="true"></span>
													<?php esc_html_e('View Event', 'mage-eventpress'); ?>
												</a>
											<?php endif; ?>
								...
								<div class="mpwem-event-wizard__content">
									<div class="mpwem-wizard-panels">
										...
										<section class="mpwem-wizard-panel mp_tab_item" data-tab-item="#mpwem_wizard_tickets">
											<div class="mpwem-event-wizard__grid">
												<div class="mpwem-event-wizard__main">
													<div class="mpwem-card" id="mpwem_event_mode_card">
														<div class="mpwem-card__head">
															<h2><?php esc_html_e('Registration & Tickets Settings', 'mage-eventpress'); ?></h2>
															<p><?php esc_html_e('Configure how users register or purchase tickets for this event.', 'mage-eventpress'); ?></p>
														</div>
														<div class="mpwem-card__body">
															<div class="mpwem-field">
																<span class="mpwem-field__label"><?php esc_html_e('Event Mode', 'mage-eventpress'); ?></span>
																<?php 
													...
													<div id="mpwem_wizard_ticket_details_section">
												...
												<aside class="mpwem-event-wizard__sidebar" id="mpwem_wizard_tickets_sidebar">
													<div class="mpwem-card mpwem-card--help" id="mpwem_wizard_mode_help_card" style="margin-bottom: 20px;">
														<div class="mpwem-card__head">
															<h2><?php esc_html_e('Event Mode Manual', 'mage-eventpress'); ?></h2>
															<p><?php esc_html_e('Learn what each event mode is designed for:', 'mage-eventpress'); ?></p>
														</div>
														<div class="mpwem-card__body">
													...
													<div class="mpwem-card mpwem-card--help mpwem-card--help-pricing" id="mpwem_wizard_pricing_help_card">
														<div class="mpwem-card__head">
